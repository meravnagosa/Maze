...
//        this.setLocationRelativeTo(null);
    ...
    private boolean findDfsPath(){
        boolean result=false;
        Stack<Cell> stack=new Stack<>();
        Cell root = new Cell(startRow,startColumn);
        stack.push(root);
        while(!stack.empty()){
            ...
            if (visited[row][col]==false){
                setSquareAsVisited(row,col,true);
                //adding neighbors
                if (col+1!= values.length&&values[row][col+1]!=Definitions.OBSTACLE)
                    stack.push(new Cell(row , col+1));
                if (row+1!= values.length&&values[row+1][col]!=Definitions.OBSTACLE)
